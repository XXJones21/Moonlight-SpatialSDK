...
// Game activity not available in Spatial SDK port
...
public class UiHelper {
        ...
        if (crashCount != 0 && crashCount != lastNotifiedCrashCount) {
            if (crashCount % 3 == 0) {
                ...
                Dialog.displayDialog(activity,
                        "Decoding Settings Reset",
                        "Decoding settings have been reset due to repeated crashes",
                        new Runnable() {
                            @Override
                            public void run() {
    ...
    public static void displayDeletePcConfirmationDialog(Activity parent, ComputerDetails computer, final Runnable onYes, final Runnable onNo) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(parent);
        builder.setMessage("Are you sure you want to delete this PC?")
