...
// Portal adaptation: credentials are device-local Keychain items.
+ (NSDictionary*) queryForItem:(NSString*)item {
    return @{(__bridge id)kSecClass:(__bridge id)kSecClassGenericPassword,
             (__bridge id)kSecAttrService:@"MoonlightPortal.Identity",(__bridge id)kSecAttrAccount:item};
}
+ (NSData*) readCryptoObject:(NSString*)item {
    NSMutableDictionary *query=[[self queryForItem:item] mutableCopy];
...
+ (void) writeCryptoObject:(NSString*)item data:(NSData*)data {
    ...
    if(status!=errSecSuccess) @throw [NSException exceptionWithName:@"CredentialStorage" reason:@"Unable to store Moonlight client identity in Keychain" userInfo:nil];
}
+ (void) resetIdentity {
    @synchronized(self) {
        for(NSString *item in @[@"client.key",@"client.crt",@"client.p12"])
            SecItemDelete((__bridge CFDictionaryRef)[self queryForItem:item]);
        key=nil;cert=nil;p12=nil;
    }
}
