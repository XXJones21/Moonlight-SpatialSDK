...
// HelpActivity not available in Spatial SDK port
...
public class HelpLauncher {
    public static void launchUrl(Context context, String url) {
        ...
        // HelpActivity not available in Spatial SDK - just try to open URL directly
        // If that fails, log the URL
        try {
            Intent i = new Intent(Intent.ACTION_VIEW);
            i.setData(Uri.parse(url));
            context.startActivity(i);
        } catch (Exception e) {
            // HelpActivity not available - URL cannot be opened
            android.util.Log.w("HelpLauncher", "Cannot open URL: " + url);
        }
    }
