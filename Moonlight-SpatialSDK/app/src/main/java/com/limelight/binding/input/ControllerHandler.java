...
// Optional dependencies - may not be available in Spatial SDK port
...
public class ControllerHandler implements InputManager.InputDeviceListener, UsbDriverListener {
    ...
    private final Object gestures; // GameGestures - optional, not available in Spatial SDK
    private final InputManager inputManager;
    private final Vibrator deviceVibrator;
    private final VibratorManager deviceVibratorManager;
    private final SensorManager deviceSensorManager;
    private final Object sceManager; // SceManager - optional Shield Controller Extensions
    ...
    public ControllerHandler(Activity activityContext, NvConnection conn, Object gestures, PreferenceConfiguration prefConfig) {
        ...
        // SceManager (Shield Controller Extensions) is optional and not available in Spatial SDK
        Object sceManagerInstance = null;
        try {
            Class<?> sceManagerClass = Class.forName("org.cgutman.shieldcontrollerextensions.SceManager");
            sceManagerInstance = sceManagerClass.getConstructor(Activity.class).newInstance(activityContext);
            sceManagerClass.getMethod("start").invoke(sceManagerInstance);
        } catch (Exception e) {
            // Shield Controller Extensions not available - continue without it
            sceManagerInstance = null;
        }
        this.sceManager = sceManagerInstance;
    ...
    public void destroy() {
        ...
        if (sceManager != null) {
            try {
                sceManager.getClass().getMethod("stop").invoke(sceManager);
            } catch (Exception e) {
                // SCE not available or failed
            }
        }
    ...
    private void sendControllerBatteryPacket(InputDeviceContext context) {
        ...
        else if (sceManager != null) {
            // On the SHIELD Android TV, we can use a proprietary API to access battery/charge state.
            // This is optional and may not be available in Spatial SDK.
            try {
                boolean isRecognized = (Boolean) sceManager.getClass().getMethod("isRecognizedDevice", InputDevice.class).invoke(sceManager, context.inputDevice);
                if (isRecognized) {
                    int batteryPercentage = (Integer) sceManager.getClass().getMethod("getBatteryPercentage", InputDevice.class).invoke(sceManager, context.inputDevice);
                    ...
                    Object connectionType = sceManager.getClass().getMethod("getConnectionType", InputDevice.class).invoke(sceManager, context.inputDevice);
                    Object chargingState = sceManager.getClass().getMethod("getChargingState", InputDevice.class).invoke(sceManager, context.inputDevice);

                    // Use reflection to access enum values
                    Class<?> sceConnectionTypeClass = Class.forName("org.cgutman.shieldcontrollerextensions.SceConnectionType");
                    Class<?> sceChargingStateClass = Class.forName("org.cgutman.shieldcontrollerextensions.SceChargingState");
                    Object wired = Enum.valueOf((Class<Enum>) sceConnectionTypeClass, "WIRED");
                    Object both = Enum.valueOf((Class<Enum>) sceConnectionTypeClass, "BOTH");
                    Object wireless = Enum.valueOf((Class<Enum>) sceConnectionTypeClass, "WIRELESS");
                    Object notCharging = Enum.valueOf((Class<Enum>) sceChargingStateClass, "NOT_CHARGING");
                    Object charging = Enum.valueOf((Class<Enum>) sceChargingStateClass, "CHARGING");

                    // We can make some assumptions about charge state based on the connection type
                    if (connectionType == wired || connectionType == both) {
                        if (batteryPercentage == 100) {
                            currentBatteryStatus = BatteryState.STATUS_FULL;
                        }
                        else if (chargingState == notCharging) {
                ...
                // SCE API not available or failed - skip battery reporting
                return;
            }
        }
        else {
        ...
        for (int i = 0; i < inputDeviceContexts.size(); i++) {
            ...
            if (deviceContext.controllerNumber == controllerNumber) {
                ...
                // On Shield devices, we can use their special API to rumble Shield controllers
                    ...
                    try {
                        Boolean result = (Boolean) sceManager.getClass().getMethod("rumble", InputDevice.class, short.class, short.class)
                                .invoke(sceManager, deviceContext.inputDevice, deviceContext.lowFreqMotor, deviceContext.highFreqMotor);
    ...
    class InputDeviceContext extends GenericControllerContext {
        ...
        public void sendControllerArrival() {
            ...
            if (type != MoonBridge.LI_CTYPE_PS && sensorManager != null) {
                ...
                Toast.makeText(activityContext, "Controller type changed", Toast.LENGTH_LONG).show();
            ...
            if (sceManager != null) {
                try {
                    Boolean isRecognized = (Boolean) sceManager.getClass().getMethod("isRecognizedDevice", InputDevice.class)
                            .invoke(sceManager, inputDevice);
                    if (isRecognized != null && isRecognized) {
