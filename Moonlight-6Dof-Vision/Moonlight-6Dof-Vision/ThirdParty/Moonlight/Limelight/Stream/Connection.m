...
{
    id<ConnectionCallbacks> callbacks = _callbacks;
...
void ClStageFailed(int stage, int errorCode)
{
    id<ConnectionCallbacks> callbacks = _callbacks;
    dispatch_async(dispatch_get_main_queue(), ^{
        [callbacks stageFailed:LiGetStageName(stage) withError:errorCode portTestFlags:LiGetPortFlagsFromStage(stage)];
    });
}
...
{
    id<ConnectionCallbacks> callbacks = _callbacks;
    id<AnyVideoDecoderRenderer> targetRenderer = renderer;
    dispatch_async(dispatch_get_main_queue(), ^{
        [targetRenderer setHdrMode:enabled];
        [callbacks setHdrMode:enabled];
    });
}
...
void ClRumbleTriggers(uint16_t controllerNumber, uint16_t leftTriggerMotor, uint16_t rightTriggerMotor)
{
    id<ConnectionCallbacks> callbacks = _callbacks;
    dispatch_async(dispatch_get_main_queue(), ^{
        [callbacks rumbleTriggers:controllerNumber leftTrigger:leftTriggerMotor rightTrigger:rightTriggerMotor];
    });
}
...
void ClSetControllerLED(uint16_t controllerNumber, uint8_t r, uint8_t g, uint8_t b)
{
    id<ConnectionCallbacks> callbacks = _callbacks;
    dispatch_async(dispatch_get_main_queue(), ^{
        [callbacks setControllerLed:controllerNumber r:r g:g b:b];
    });
}
...
-(void) terminateWithCompletion:(void (^)(void))completion
{
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED,0), ^{
        [initLock lock];
        LiStopConnection(); // Completion means teardown actually finished; no overlapping globals.
...
-(id) initWithConfig:(StreamConfiguration*)config renderer:(id<AnyVideoDecoderRenderer> __strong)myRenderer connectionCallbacks:(id<ConnectionCallbacks>)callbacks
{
    ...
    _streamConfig.colorRange = config.colorRange;
    _streamConfig.colorSpace = config.colorSpace;
...
{
    [initLock lock];
    if (self.isCancelled) { [initLock unlock]; return; }
