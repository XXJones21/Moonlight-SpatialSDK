...
enum StereoPreview {
    @MainActor static func makeTexture(stereo: Bool) throws -> TextureResource {
        let image = UIGraphicsImageRenderer(size: CGSize(width: stereo ? 1024 : 512, height: 288)).image { context in
            for eye in 0..<(stereo ? 2 : 1) {
                ...
                (stereo ? (eye == 0 ? "LEFT EYE" : "RIGHT EYE") : "MOONLIGHT").draw(at: CGPoint(x: left + 150, y: 115), withAttributes: attributes)
